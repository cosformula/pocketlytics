  ...
  try {
    ...
    try {
      const response = await fetch(validatedData.url, {
        headers: {
          "User-Agent": "Mozilla/5.0 (compatible; PocketlyticsBot/1.0; +https://pocketlytics.local)",
        },
...
Identify:
1. All analytics platforms (Google Analytics, Mixpanel, Amplitude, Plausible, Pocketlytics, etc.)
