...
function getBrowserIconPath(browser: string): string {
  return BROWSER_TO_LOGO[browser] ? `https://app.pocketlytics.local/browsers/${BROWSER_TO_LOGO[browser]}` : "";
}

function getOSIconPath(os: string): string {
  return OS_TO_LOGO[os] ? `https://app.pocketlytics.local/operating-systems/${OS_TO_LOGO[os]}` : "";
}
    ...
    const fetchSessions = async () => {
      try {
        const response = await fetch(
          "https://demo.pocketlytics.local/api/sites/1/sessions?past_minutes_start=120&past_minutes_end=0&filters=[]&page=1&limit=100"
        );
