---
title: "Shopify"
description: "Integrate Pocketlytics with your Shopify store to capture events, track conversions, and analyze user behavior."
---
...
Pocketlytics can be integrated with your Shopify store to capture events, track conversions, and analyze user behavior.

## How to Add Pocketlytics to Shopify

<Steps>
<Step>
### Retrieve Your Tracking Script

Navigate to your Pocketlytics dashboard to obtain your code snippet.

```html
<script
  src="https://app.pocketlytics.local/api/script.js"
...
To get the most out of Pocketlytics with Shopify, you should track key e-commerce events. This typically involves adding small Liquid and JavaScript snippets to your theme files.
...
<script type="text/javascript">
  if (typeof window.pocketlytics !== 'undefined' && {{ product | json }}) {
    const pocketlyticsProduct = {{ product | json }};
    const pocketlyticsCurrentVariant = {{ product.selected_or_first_available_variant | json }};

    window.pocketlytics.event('view_item', {
      item_id: pocketlyticsCurrentVariant.sku || pocketlyticsCurrentVariant.id,
      item_name: pocketlyticsProduct.title,
      item_variant: pocketlyticsCurrentVariant.title === 'Default Title' ? null : pocketlyticsCurrentVariant.title,
      price: parseFloat(pocketlyticsCurrentVariant.price) / 100, // Price is in cents
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const trackAddToCart = function() {
    if (typeof window.pocketlytics !== 'undefined' && {{ product | json }}) {
      ...
      window.pocketlytics.event('add_to_cart', {
        item_id: pocketlyticsCurrentVariant.sku || pocketlyticsCurrentVariant.id,
        item_name: pocketlyticsProduct.title,
        item_variant: pocketlyticsCurrentVariant.title === 'Default Title' ? null : pocketlyticsCurrentVariant.title,
        price: parseFloat(pocketlyticsCurrentVariant.price) / 100,
        currency: {{ cart.currency.iso_code | json }},
...
Shopify's checkout process is on a separate domain (`checkout.shopify.com`), and direct script injection into the checkout pages (beyond the "Order status" page) is limited for most plans. However, Pocketlytics's script, if loaded on your main store, should persist session information.
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (checkoutButton && typeof window.pocketlytics !== 'undefined' && {{ cart | json }}) {
    checkoutButton.addEventListener('click', function() {
      const pocketlyticsCart = {{ cart | json }};
      let items = [];
      pocketlyticsCart.items.forEach(function(item) {
        items.push({
      ...
      window.pocketlytics.event('begin_checkout', {
        currency: pocketlyticsCart.currency,
        value: parseFloat(pocketlyticsCart.total_price) / 100,
        items: items
      });
    });
  }
...
<script type="text/javascript">
  if (typeof window.pocketlytics !== 'undefined' && {{ checkout | json }}) {
    const pocketlyticsCheckout = {{ checkout | json }};
    let items = [];
    pocketlyticsCheckout.line_items.forEach(function(item) {
      items.push({
        item_id: item.sku || item.variant_id,
    ...
    window.pocketlytics.event('purchase', {
      transaction_id: pocketlyticsCheckout.order_number,
      value: parseFloat(pocketlyticsCheckout.total_price),
      tax: parseFloat(pocketlyticsCheckout.tax_price),
      shipping: parseFloat(pocketlyticsCheckout.shipping_price),
      currency: pocketlyticsCheckout.currency,
      items: items
    });
  }
</script>
...
*   **Currency Conversion**: Prices in Shopify Liquid objects are often in cents. Ensure you divide by 100 to get the decimal value for Pocketlytics.
...
*   **Shopify Customer Events**: For more robust server-side or client-side event tracking, especially for `add_to_cart` and `begin_checkout` without relying on DOM manipulation, explore [Shopify Customer Events](https://shopify.dev/docs/api/customer-events). This allows you to subscribe to events like "product added to cart" or "checkout started" and send data to Pocketlytics via a custom pixel or by triggering client-side JavaScript. This is a more advanced but often more reliable method.
*   **Testing**: Thoroughly test each event in your Pocketlytics dashboard after implementation. Use your browser's developer console to check for JavaScript errors.