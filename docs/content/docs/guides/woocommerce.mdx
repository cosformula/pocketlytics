---
title: WooCommerce
description: Integrate Pocketlytics Analytics with your WooCommerce store
---
...
This guide explains how to integrate Pocketlytics with your WooCommerce store (running on WordPress) to track e-commerce events like product views, add to carts, and purchases, in addition to standard page view analytics.

## Prerequisites

1.  **Pocketlytics Installed on WordPress**: Ensure you have already installed the Pocketlytics tracking script on your WordPress site using one of the methods described in our [WordPress integration guide](/docs/guides/wordpress). This guide focuses on adding e-commerce event tracking.
...
WooCommerce provides various action hooks that allow you to trigger JavaScript when specific e-commerce events occur. We'll use these to send data to Pocketlytics.
...
add_action( 'woocommerce_after_single_product_summary', 'pocketlytics_track_view_product', 10 );
function pocketlytics_track_view_product() {
    ...
    <script type="text/javascript">
        if (typeof window.pocketlytics !== 'undefined') {
            window.pocketlytics.event('view_item', <?php echo json_encode($product_data); ?>);
        }
    </script>
    <?php
}
...
```php
// Add to your child theme's functions.php or a custom plugin
add_action( 'woocommerce_after_add_to_cart_button', 'pocketlytics_track_add_to_cart_script' );
function pocketlytics_track_add_to_cart_script() {
    global $product;
    if ( ! $product instanceof WC_Product ) {
        return;
    }
        ...
        if (form) {
            form.addEventListener('submit', function() {
                ...
                if (typeof window.pocketlytics !== 'undefined') {
                    ...
                    window.pocketlytics.event('add_to_cart', eventData);
                }
            });
        }
    });
...
add_action( 'wp_footer', 'pocketlytics_ajax_add_to_cart_tracker' );
function pocketlytics_ajax_add_to_cart_tracker() {
    if ( is_admin() ) return;
    ?>
    <script type="text/javascript">
        jQuery(document.body).on('added_to_cart', function(event, fragments, cart_hash, $button) {
            if (typeof window.pocketlytics !== 'undefined' && $button && $button.length) {
...
```php
// Add to your child theme's functions.php or a custom plugin
add_action( 'woocommerce_before_checkout_form', 'pocketlytics_track_begin_checkout', 5 );
function pocketlytics_track_begin_checkout() {
    ...
    <script type="text/javascript">
        if (typeof window.pocketlytics !== 'undefined') {
            window.pocketlytics.event('begin_checkout', <?php echo json_encode($checkout_data); ?>);
        }
    </script>
    <?php
}
...
```php
// Add to your child theme's functions.php or a custom plugin
add_action( 'woocommerce_thankyou', 'pocketlytics_track_purchase', 10, 1 );
function pocketlytics_track_purchase( $order_id ) {
    if ( ! $order_id ) {
        return;
    }
    ...
    <script type="text/javascript">
        if (typeof window.pocketlytics !== 'undefined') {
            window.pocketlytics.event('purchase', <?php echo json_encode($purchase_data); ?>);
        }
    </script>
    <?php
}
...
*   **Script Loading Order**: Ensure the main Pocketlytics tracking script (added via the WordPress integration guide) is loaded before these event-specific scripts attempt to call `window.pocketlytics`. Placing the main script in the `<head>` usually ensures this.
...
By implementing these tracking snippets, you can gain valuable insights into your WooCommerce store's performance using Pocketlytics.