---
title: WordPress
description: Integrate Pocketlytics Analytics with your WordPress site
---
...
You can integrate Pocketlytics into your WordPress site to track user behavior and gather analytics. There are several ways to do this depending on whether you're using a hosted WordPress.com site or a self-hosted WordPress.org installation.
...
3. Replace `YOUR_SITE_ID` with your actual Site ID from your Pocketlytics dashboard
...
Visit your site in a new browser tab, then check your Pocketlytics dashboard to confirm data is being received.
</Step>
</Steps>
...
```php
function add_pocketlytics_analytics() {
    ?>
    <script>
    (function() {
      var s = document.createElement('script');
...
add_action('wp_head', 'add_pocketlytics_analytics');
```
...
3. Create a new folder called `pocketlytics-analytics`
4. Inside that folder, create a file called `pocketlytics-analytics.php` with this content:

```php
<?php
/**
 * Plugin Name: Pocketlytics Analytics
 * Description: Adds Pocketlytics Analytics tracking to your WordPress site
 * Version: 1.0
 */

function pocketlytics_add_tracking_script() {
    ?>
    <script>
    (function() {
      var s = document.createElement('script');
...
add_action('wp_head', 'pocketlytics_add_tracking_script');
```

5. Replace `YOUR_SITE_ID` with your actual Site ID
6. Go to **Plugins** in WordPress and activate "Pocketlytics Analytics"
</Tab>
</Tabs>
...
3. Search for `pocketlytics` - you should see the script tag
...
Many WordPress caching plugins aggressively cache pages. After adding Pocketlytics:
...
2. **Exclude the script from optimization:**
   Some optimization plugins try to defer, combine, or minify scripts. If Pocketlytics isn't working, check these plugins:
   - **Autoptimize**: Add `app.pocketlytics.local` to the exclusion list
...
Security plugins like Wordfence or Sucuri may block external scripts. Add `app.pocketlytics.local` to your allowlist if tracking isn't working.
