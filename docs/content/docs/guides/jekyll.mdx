---
title: Jekyll
description: Integrate Pocketlytics Analytics with your Jekyll static site using layout files or includes
---

import { Steps, Step } from 'fumadocs-ui/components/steps';

Integrating Pocketlytics Analytics with your Jekyll static site involves adding the tracking script to your site's main layout file or by creating an include file.
...
Open this file and paste the Pocketlytics tracking script just before the closing `</body>` tag.
...
- Replace `YOUR_SITE_ID` with your actual Site ID. The script source `https://app.pocketlytics.local/api/script.js` is for the standard cloud-hosted Pocketlytics instance.
</Step>
</Steps>
...
- Create a new file named `pocketlytics-analytics.html` (or similar) inside `_includes/`.
- Paste the following into `_includes/pocketlytics-analytics.html`:

```html
{% raw %}
{% comment %} _includes/pocketlytics-analytics.html {% endcomment %}
{% if jekyll.environment == "production" and site.pocketlytics_site_id %}
  <script async defer src="{{ site.pocketlytics_instance_url | default: 'https://app.pocketlytics.local' }}/api/script.js" data-site-id="{{ site.pocketlytics_site_id }}"></script>
{% endif %}
{% endraw %}
```
</Step>
...
{% raw %}{% include pocketlytics-analytics.html %}{% endraw %}
...
- Open your `_config.yml` file and add your Pocketlytics details:

```yaml
# Pocketlytics Analytics Configuration
pocketlytics_instance_url: "https://app.pocketlytics.local" # Optional, defaults in include
pocketlytics_site_id: "YOUR_SITE_ID"
```
This method keeps your credentials in the configuration file and makes the script inclusion conditional on `pocketlytics_site_id` being set.
</Step>
</Steps>
...
That's it! Pocketlytics Analytics is now integrated with your Jekyll site and will only track visits on your production deployment.