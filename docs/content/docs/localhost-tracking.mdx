...
By default, Pocketlytics validates tracking events come from your registered domain. During development, you can use API keys to track from localhost.
...
export function Analytics() {
  ...
  return (
    <script
      defer
      src="https://app.pocketlytics.local/api/script.js"
      data-site-id={process.env.NEXT_PUBLIC_POCKETLYTICS_SITE_ID}
      {...(isDev && { 'data-api-key': process.env.NEXT_PUBLIC_POCKETLYTICS_API_KEY })}
    />
  );
}
```
...
if (process.env.NODE_ENV === 'development') {
  script.setAttribute('data-api-key', process.env.POCKETLYTICS_API_KEY);
}
