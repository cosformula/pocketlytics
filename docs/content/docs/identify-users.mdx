...
description: How to identify and track logged-in users with Pocketlytics
---
...
Pocketlytics allows you to identify logged-in users and associate their activity with a persistent identity across devices and sessions. This enables powerful user-level analytics, including tracking user journeys, understanding retention, and personalizing experiences.

![identify users](/docs/users.png)

## How It Works

By default, Pocketlytics assigns an anonymous ID to each visitor based on their device fingerprint. When you call `identify()`, you link this anonymous visitor to a known user ID, allowing you to:
...
When identifying users, certain trait fields are treated specially by Pocketlytics:
...
### `window.pocketlytics.identify(userId, traits?)`
...
- `traits` (Object, Optional): An object containing user metadata like email, name, username, or any custom fields. This data is stored in Pocketlytics and can be used to identify users in the dashboard. Setting a trait to `null` removes it from the user's profile.

```javascript
// Identify a user when they log in (without traits)
window.pocketlytics.identify("user_12345");
...
### `window.pocketlytics.setTraits(traits)`
...
### `window.pocketlytics.clearUserId()`
...
function handleLogin(userData) {
  ...
  window.pocketlytics.identify(userData.id, {
  ...
  // Track login event
  window.pocketlytics.event("User Login", {
    method: "email"
  });
}
...
function handlePlanUpgrade(newPlan) {
  ...
  // Track upgrade event
  window.pocketlytics.event("Plan Upgraded", {
    new_plan: newPlan
  });
}
...
function handleLogout() {
  // Track logout event
  window.pocketlytics.event("User Logout");
...
function checkUserStatus() {
  const userId = window.pocketlytics.getUserId();
  if (userId) {
