...
1. Navigate to **Settings → Account** in your Pocketlytics dashboard
...
<Tab value="Pageview">
```bash
curl -X POST 'https://app.pocketlytics.local/api/track' \
...
const trackEvent = async (eventData) => {
  const response = await fetch('https://app.pocketlytics.local/api/track', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${process.env.POCKETLYTICS_API_KEY}`,
    },
    body: JSON.stringify({
      site_id: process.env.POCKETLYTICS_SITE_ID,
      ...eventData
    })
  });
...
def track_event(event_data):
    response = requests.post(
        'https://app.pocketlytics.local/api/track',
        headers={
            'Content-Type': 'application/json',
            'Authorization': f"Bearer {os.getenv('POCKETLYTICS_API_KEY')}"
        },
        json={
            'site_id': os.getenv('POCKETLYTICS_SITE_ID'),
            **event_data
        }
    )
    return response.json()
...
func trackEvent(event TrackingEvent) error {
    event.SiteID = os.Getenv("POCKETLYTICS_SITE_ID")
    ...
    req.Header.Set("Authorization", "Bearer "+os.Getenv("POCKETLYTICS_API_KEY"))
...
public class PocketlyticsTracker {
    private static final String API_URL = "https://app.pocketlytics.local/api/track";
    private static final String API_KEY = System.getenv("POCKETLYTICS_API_KEY");
    private static final String SITE_ID = System.getenv("POCKETLYTICS_SITE_ID");
    ...
    public static void main(String[] args) throws Exception {
        PocketlyticsTracker tracker = new PocketlyticsTracker();
...
class PocketlyticsTracker {
    private $apiUrl = 'https://app.pocketlytics.local/api/track';
    private $apiKey;
    private $siteId;

    public function __construct() {
        $this->apiKey = getenv('POCKETLYTICS_API_KEY');
        $this->siteId = getenv('POCKETLYTICS_SITE_ID');
    }
...
// Usage
$tracker = new PocketlyticsTracker();
...
class PocketlyticsTracker
  API_URL = 'https://app.pocketlytics.local/api/track'

  def initialize
    @api_key = ENV['POCKETLYTICS_API_KEY']
...
# Usage
tracker = PocketlyticsTracker.new
...
public class PocketlyticsTracker
{
    ...
    private const string ApiUrl = "https://app.pocketlytics.local/api/track";

    public PocketlyticsTracker()
    {
        _httpClient = new HttpClient();
        _apiKey = Environment.GetEnvironmentVariable("POCKETLYTICS_API_KEY");
        _siteId = Environment.GetEnvironmentVariable("POCKETLYTICS_SITE_ID");
    ...
    {
        var tracker = new PocketlyticsTracker();
...
impl PocketlyticsTracker {
    fn new() -> Self {
        Self {
            client: reqwest::Client::new(),
            api_key: env::var("POCKETLYTICS_API_KEY").expect("POCKETLYTICS_API_KEY not set"),
            site_id: env::var("POCKETLYTICS_SITE_ID").expect("POCKETLYTICS_SITE_ID not set"),
            api_url: "https://app.pocketlytics.local/api/track".to_string(),
        }
    }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let tracker = PocketlyticsTracker::new();
...
class PocketlyticsTracker {
    static let shared = PocketlyticsTracker()

    private let apiUrl = "https://app.pocketlytics.local/api/track"
    private let apiKey: String
    private let siteId: String

    init() {
        // Ensure API key and site ID are available
        guard let apiKey = ProcessInfo.processInfo.environment["POCKETLYTICS_API_KEY"],
              !apiKey.isEmpty,
              let siteId = ProcessInfo.processInfo.environment["POCKETLYTICS_SITE_ID"],
              !siteId.isEmpty else {
            fatalError("POCKETLYTICS_API_KEY and POCKETLYTICS_SITE_ID environment variables must be set")
        }
        self.apiKey = apiKey
        self.siteId = siteId
    }
    ...
    func trackEvent(_ eventData: [String: Any]) async throws {
        ...
        guard let httpResponse = response as? HTTPURLResponse,
            ...
            throw NSError(domain: "PocketlyticsTracker", code: 1, userInfo: [NSLocalizedDescriptionKey: "Tracking failed"])
        }
    }
...
Task {
    // Track a pageview
    try await PocketlyticsTracker.shared.trackEvent([
        "type": "pageview",
        "pathname": "/api/users",
        "hostname": "api.example.com",
        "page_title": "User API Endpoint"
...
class PocketlyticsTracker(private val context: Context) {
    companion object {
        private const val API_URL = "https://app.pocketlytics.local/api/track"
        private val JSON = "application/json; charset=utf-8".toMediaType()
    }

    private val client = OkHttpClient()
    private val apiKey = BuildConfig.POCKETLYTICS_API_KEY // Set in build.gradle
    private val siteId = BuildConfig.POCKETLYTICS_SITE_ID
...
class MainActivity : AppCompatActivity() {
    private val tracker = PocketlyticsTracker(this)
...
class PocketlyticsTracker {
  static final PocketlyticsTracker _instance = PocketlyticsTracker._internal();
  factory PocketlyticsTracker() => _instance;
  PocketlyticsTracker._internal();

  static const String _apiUrl = 'https://app.pocketlytics.local/api/track';
  static const String _apiKey = String.fromEnvironment('POCKETLYTICS_API_KEY');
  static const String _siteId = String.fromEnvironment('POCKETLYTICS_SITE_ID');
...
class _MyAppState extends State<MyApp> {
  final tracker = PocketlyticsTracker();
...
<Callout type="info">
**Browser Tracking**: For standard browser-based tracking, use the [Pocketlytics Web SDK](/docs/sdks/web) instead. The HTTP API is designed for server-side applications, mobile apps, and custom integrations.
</Callout>
