...
description: Set up Pocketlytics with your own Nginx installation instead of using the built-in Caddy server
---
...
This guide shows you how to set up Pocketlytics with your own Nginx installation instead of using the built-in Caddy server.
...
- Pocketlytics running with `--no-webserver` flag

## Setup Steps

<Steps>
<Step>
### Run Pocketlytics without Caddy

First, set up Pocketlytics to expose ports without the built-in webserver:
...
If you need to use custom ports for Pocketlytics:
...
**502 Bad Gateway**: Check if Pocketlytics services are running:
```bash
docker compose ps
docker compose logs
```
