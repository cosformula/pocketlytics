...
description: Manual setup guide for Pocketlytics using Docker Compose without the setup script
---
...
This guide is for users who want to manually set up Pocketlytics using Docker Compose without the setup script. This is useful if you already have a reverse proxy setup (like Nginx, Traefik, or Coolify), or if you want more control over the configuration.
...
```bash
git clone https://github.com/cosformula/pocketlytics.git
cd pocketlytics
```
</Step>
...
services:
  backend:
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.pocketlytics-api.rule=Host(`your.domain.com`) && PathPrefix(`/api`)"
      - "traefik.http.routers.pocketlytics-api.tls.certresolver=letsencrypt"
      - "traefik.http.services.pocketlytics-api.loadbalancer.server.port=3001"
  
  client:
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.pocketlytics-client.rule=Host(`your.domain.com`)"
      - "traefik.http.routers.pocketlytics-client.tls.certresolver=letsencrypt"
      - "traefik.http.services.pocketlytics-client.loadbalancer.server.port=3002"
```
</Tab>
</Tabs>
</Step>
...
Pocketlytics consists of these services:
...
services:
  ...
  backend:
    image: ghcr.io/cosformula/pocketlytics-backend:${IMAGE_TAG:-latest}
    ...
    labels:
      ...
      - traefik.http.routers.pocketlytics-backend.entrypoints=https
      - traefik.http.routers.pocketlytics-backend.tls=true
      - traefik.http.routers.pocketlytics-backend.rule=Host(`${DOMAIN_NAME}`) && (Path(`/api`) || PathPrefix(`/api/`))
      - traefik.http.services.pocketlytics-backend.loadbalancer.server.port=3001
      - traefik.http.routers.pocketlytics-backend.priority=100
      - traefik.http.middlewares.pocketlytics-forward-headers.headers.customrequestheaders.X-Forwarded-Proto=https
      - traefik.http.middlewares.pocketlytics-forward-headers.headers.customrequestheaders.X-Forwarded-Host=${DOMAIN_NAME}
      - traefik.http.routers.pocketlytics-backend.middlewares=pocketlytics-forward-headers

  client:
    image: ghcr.io/cosformula/pocketlytics-client:${IMAGE_TAG:-latest}
    ...
    labels:
      - traefik.enable=true
      - traefik.docker.network=coolify
      - traefik.http.routers.pocketlytics-client.entrypoints=https
      - traefik.http.routers.pocketlytics-client.tls=true
      - traefik.http.routers.pocketlytics-client.rule=Host(`${DOMAIN_NAME}`)
      - traefik.http.services.pocketlytics-client.loadbalancer.server.port=3002
      - traefik.http.routers.pocketlytics-client.priority=10
...
To deploy Pocketlytics on Coolify, you need to:
...
- `pocketlytics_clickhouse-data`: Analytics data
- `pocketlytics_postgres-data`: User accounts and site configurations
...
These guides provide step-by-step instructions for integrating Pocketlytics with your existing infrastructure.
