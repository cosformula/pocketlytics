---
title: Track Events
description: How to track custom events with Pocketlytics
---

The script exposes a global object `window.pocketlytics` with functions for manual tracking of events, as well as the ability to track events using data attributes on HTML elements.
...
<button
  data-pocketlytics-event="purchase_click"
  data-pocketlytics-prop-product="premium"
  data-pocketlytics-prop-price="49.99"
>
  Buy Now
</button>
```
...
### `window.pocketlytics.event(eventName, properties)`
...
window.pocketlytics.event("Signup Button Clicked");

// Track a custom event with properties
window.pocketlytics.event("Item Added To Cart", {
  itemId: "PROD123",
  price: 49.99,
  category: "Clothing"
});
...
Put this as `pocketlytics.d.ts` anywhere in your project to avoid having to do `(window as any).pocketlytics.event()` everywhere.
