...
description: Configure Express.js middleware to proxy Pocketlytics tracking requests
---
...
Express.js makes it easy to proxy Pocketlytics tracking using middleware. This guide shows how to set up a reverse proxy using `http-proxy-middleware`.

## Overview

The `http-proxy-middleware` package provides a simple way to proxy requests in Express applications, making it perfect for routing analytics requests to Pocketlytics servers.

**What you'll achieve:**
- Proxy all Pocketlytics endpoints through your Express app
...
const app = express();
const POCKETLYTICS_HOST = process.env.POCKETLYTICS_HOST || 'https://app.pocketlytics.local';

// Pocketlytics Analytics Proxy
// Proxy all /analytics/* requests to Pocketlytics
app.use('/analytics', createProxyMiddleware({
  target: POCKETLYTICS_HOST,
  changeOrigin: true,
  pathRewrite: {
...
The `http-proxy-middleware` intercepts requests matching the specified paths and forwards them to Pocketlytics:

1. Request to `/analytics/script.js` is intercepted
2. Path is rewritten to `/api/script.js`
3. Request is forwarded to `https://app.pocketlytics.local/api/script.js`
...
**Problem**: 404 errors on Pocketlytics endpoints.
