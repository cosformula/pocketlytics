...
description: Use Cloudflare Workers to proxy Pocketlytics tracking at the edge
---
...
Cloudflare Workers provide a serverless way to proxy Pocketlytics tracking at the edge, close to your users worldwide. This delivers excellent performance with minimal configuration.
...
2. Name it `pocketlytics-proxy` (or your preferred name)
...
```javascript
// Cloudflare Worker for Pocketlytics Analytics Proxy

const POCKETLYTICS_HOST = 'https://app.pocketlytics.local';
...
async function handleRequest(request) {
  ...
  try {
    return await proxyToPocketlytics(request, url);
  } catch (error) {
...
async function proxyToPocketlytics(request, url) {
  // Map paths: /analytics/* → /api/*
  let pocketlyticsPath = url.pathname.replace('/analytics/', '/api/');

  // Build Pocketlytics URL
  const pocketlyticsUrl = new URL(pocketlyticsPath, POCKETLYTICS_HOST);
  pocketlyticsUrl.search = url.search; // Preserve query params
  ...
  // Build proxy request
  const proxyRequest = new Request(pocketlyticsUrl, {
...
```javascript
// Minimal Cloudflare Worker for Pocketlytics
...
async function handleRequest(request) {
  ...
  if (url.pathname === '/analytics/script.js' || url.pathname === '/analytics/track') {
    const pocketlyticsPath = url.pathname.replace('/analytics/', '/api/');
    const pocketlyticsUrl = `${POCKETLYTICS_HOST}${pocketlyticsPath}`;
...
async function handleRequest(request, env) {
  const POCKETLYTICS_HOST = env.POCKETLYTICS_HOST || 'https://app.pocketlytics.local';
  const url = new URL(request.url);
  ...
  return fetch(pocketlyticsUrl.toString(), {
    method: request.method,
    headers: proxyHeaders,
...
**Workers & Pages** → Your Worker → **Settings** → **Variables** → Add `POCKETLYTICS_HOST`
...
3. **Verify in Pocketlytics dashboard**: Data should appear
...
3. If not cached, proxies to `app.pocketlytics.local/api/script.js`
...
async function handleRequest(request) {
  ...
  try {
    const response = await proxyToPocketlytics(request, url);
...
1. Verify cache key includes important parts:
   ```javascript
   const cacheKey = new Request(pocketlyticsUrl, request);
   ```
...
# Create worker
wrangler init pocketlytics-proxy
