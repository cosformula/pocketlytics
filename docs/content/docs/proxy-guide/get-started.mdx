...
description: Framework-agnostic guide to proxy Pocketlytics tracking through your own domain
---
...
This guide covers the core concepts for setting up a Pocketlytics proxy with any reverse proxy or server framework. If your specific framework has a dedicated guide, we recommend following that instead for optimized instructions.
...
- **Your Pocketlytics instance URL**:
  - Cloud: `https://app.pocketlytics.local`
  - Self-hosted: Your Pocketlytics instance URL (e.g., `https://analytics.yourcompany.com`)
- **Your Pocketlytics site ID** (found in your Pocketlytics dashboard)

## Understanding the Endpoints

Pocketlytics uses several endpoints for tracking. You can choose between a minimal setup or full proxy:
...
This setup supports all Pocketlytics features including session replay, Web Vitals, and custom events.
...
Set up your reverse proxy to forward requests from your chosen path (e.g., `/analytics/*`) to the Pocketlytics server.

**Generic proxy configuration pattern:**

```
Your domain path          →  Pocketlytics backend
-----------------            ---------------
/analytics/script.js      →  https://app.pocketlytics.local/api/script.js
/analytics/track          →  https://app.pocketlytics.local/api/track
/analytics/replay.js      →  https://app.pocketlytics.local/api/replay.js
... and so on
```
...
Implement caching to reduce load on Pocketlytics servers and improve performance:
...
Change your script tag to load from your proxied domain instead of directly from Pocketlytics:

**Before (direct loading):**
```html
<script src="https://app.pocketlytics.local/api/script.js" async data-site-id="YOUR_SITE_ID"></script>
```
...
<Callout type="info">
**How Auto-Discovery Works**: The Pocketlytics script automatically extracts the analytics host from its own `src` attribute by splitting on `/script.js`. This means when it loads from `https://yourdomain.com/analytics/script.js`, it automatically sends all tracking data to `https://yourdomain.com/analytics/*` endpoints.
</Callout>
...
5. **Check your Pocketlytics dashboard**:
...
**CORS headers** (should be preserved from Pocketlytics):
```
...
For accurate tracking, ensure these headers are forwarded to the Pocketlytics backend:
...
<Callout type="warn">
**IP Forwarding**: Pocketlytics uses IP addresses for geolocation (country/city) and creating anonymous user identifiers (hashed IP + User Agent). If you don't forward the `X-Forwarded-For` or `X-Real-IP` header, all traffic will appear to come from your proxy server's IP.
</Callout>
...
Configure your proxy to maintain persistent connections to Pocketlytics's backend to reduce latency on repeated requests.
...
```
https://analytics.yourdomain.com/script.js  →  https://app.pocketlytics.local/api/script.js
https://analytics.yourdomain.com/track      →  https://app.pocketlytics.local/api/track
```
...
```
https://yourdomain.com/analytics/script.js  →  https://app.pocketlytics.local/api/script.js
https://yourdomain.com/analytics/track      →  https://app.pocketlytics.local/api/track
```
...
- Whitelist your Pocketlytics backend IP if possible
...
2. **CORS errors**: Ensure CORS headers are preserved from Pocketlytics backend
