...
description: Deploy Pocketlytics proxy on Vercel with Edge Network caching
---

import { Steps } from 'fumadocs-ui/components/steps';
import { Callout } from 'fumadocs-ui/components/callout';

Vercel makes it easy to proxy Pocketlytics tracking through their Edge Network. If you're using Next.js, follow the [Next.js proxy guide](/docs/proxy-guide/next-js) instead, which is optimized for Next.js applications.
...
**What you'll achieve:**
- Proxy Pocketlytics through Vercel's Edge Network
  ...
  "rewrites": [
    {
      ...
      "destination": "https://app.pocketlytics.local/api/script.js"
    },
    {
      "source": "/analytics/replay.js",
      "destination": "https://app.pocketlytics.local/api/replay.js"
    },
    {
      "source": "/analytics/metrics.js",
      "destination": "https://app.pocketlytics.local/api/metrics.js"
    },
    {
      "source": "/analytics/track",
      "destination": "https://app.pocketlytics.local/api/track"
    },
    {
      "source": "/analytics/identify",
      "destination": "https://app.pocketlytics.local/api/identify"
    },
    {
      "source": "/analytics/session-replay/record/:siteId",
      "destination": "https://app.pocketlytics.local/api/session-replay/record/:siteId"
    },
    {
      "source": "/analytics/site/tracking-config/:siteId",
      "destination": "https://app.pocketlytics.local/api/site/tracking-config/:siteId"
    }
  ]
}
```
...
```bash
git add vercel.json
git commit -m "Add Pocketlytics proxy configuration"
git push
```
...
3. **Verify in Pocketlytics dashboard**: Data should appear after 1-2 minutes
...
2. Edge server rewrites and proxies to Pocketlytics
...
**Problem**: Requests still go to `app.pocketlytics.local` directly.
...
// vercel.staging.json (staging)
{
  "rewrites": [
    {
      "source": "/analytics/script.js",
      "destination": "https://staging.pocketlytics.local/api/script.js"
    }
  ]
}
```
...
```json
// ❌ Wrong - uses redirects (user sees app.pocketlytics.local)
{
  "redirects": [
    {
      "source": "/analytics/script.js",
