...
description: Configure Astro middleware to proxy Pocketlytics tracking requests
---

import { Steps } from 'fumadocs-ui/components/steps';
import { Callout } from 'fumadocs-ui/components/callout';

Astro's middleware feature makes it simple to proxy Pocketlytics tracking. This guide shows how to set up server-side request proxying in your Astro application.
...
**What you'll achieve:**
- Proxy Pocketlytics endpoints through your Astro site
...
const POCKETLYTICS_HOST = import.meta.env.POCKETLYTICS_HOST || 'https://app.pocketlytics.local';
...
export const onRequest: MiddlewareHandler = async (context, next) => {
  ...
  if (url.pathname.startsWith('/analytics/')) {
    return proxyToPocketlytics(request, url.pathname);
  }
...
async function proxyToPocketlytics(request: Request, pathname: string): Promise<Response> {
  try {
    // Map paths: /analytics/* → /api/*
    let pocketlyticsPath = pathname.replace('/analytics/', '/api/');
    ...
    const response = await fetch(pocketlyticsUrl, {
    ...
    console.error('Pocketlytics proxy error:', error);
...
async function proxyToPocketlytics(request, pathname) {
  try {
    // Map paths: /analytics/* → /api/*
    let pocketlyticsPath = pathname.replace('/analytics/', '/api/');
...
2. Middleware proxies to `https://app.pocketlytics.local/api/script.js`
...
async function proxyToPocketlytics(request: Request, pathname: string): Promise<Response> {
  try {
    ...
    console.error('Pocketlytics proxy error:', errorDetails);
...
**Problem**: `POCKETLYTICS_HOST` is undefined.

**Solution**:
1. Check `.env` file is in project root
2. Use `import.meta.env.POCKETLYTICS_HOST` (not `process.env`)
