...
description: Configure Nginx reverse proxy to serve Pocketlytics tracking and bypass ad blockers
---
...
Nginx is a powerful web server and reverse proxy that makes it easy to proxy Pocketlytics tracking through your own domain. This guide shows how to configure Nginx to forward tracking requests to Pocketlytics servers while caching static scripts for better performance.

## Overview

Nginx's `proxy_pass` directive lets you forward requests from your domain to Pocketlytics's servers. Combined with caching, this provides excellent performance and reliability for proxying analytics.

**What you'll achieve:**
- Proxy all Pocketlytics endpoints through your domain
- Cache static scripts for better performance
- Forward necessary headers for accurate tracking
- Support all Pocketlytics features (session replay, Web Vitals, custom events)

## Prerequisites

- Nginx installed (version 1.18 or later recommended)
- SSL/TLS certificate configured (Let's Encrypt recommended)
- Your Pocketlytics instance URL:
  - **Cloud hosted**: `https://app.pocketlytics.local`
  - **Self-hosted**: Your instance URL
- Your Pocketlytics site ID (found in your dashboard)
...
server {
    ...
    # Pocketlytics Analytics Proxy
    # Main tracking script (cached)
    location = /analytics/script.js {
        proxy_pass https://app.pocketlytics.local/api/script.js;
        proxy_set_header Host app.pocketlytics.local;
    ...
    location = /analytics/replay.js {
        proxy_pass https://app.pocketlytics.local/api/replay.js;
        proxy_set_header Host app.pocketlytics.local;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location = /analytics/metrics.js {
        proxy_pass https://app.pocketlytics.local/api/metrics.js;
        proxy_set_header Host app.pocketlytics.local;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location = /analytics/track {
        proxy_pass https://app.pocketlytics.local/api/track;
        proxy_set_header Host app.pocketlytics.local;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location = /analytics/identify {
        proxy_pass https://app.pocketlytics.local/api/identify;
        proxy_set_header Host app.pocketlytics.local;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location ~ ^/analytics/session-replay/record/(.*)$ {
        proxy_pass https://app.pocketlytics.local/api/session-replay/record/$1;
        proxy_set_header Host app.pocketlytics.local;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location ~ ^/analytics/site/tracking-config/(.*)$ {
        proxy_pass https://app.pocketlytics.local/api/site/tracking-config/$1;
        proxy_set_header Host app.pocketlytics.local;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
...
<Callout type="info">
Replace `app.pocketlytics.local` with your self-hosted Pocketlytics instance URL if you're not using the cloud version.
</Callout>
...
http {
    # Define cache zone for Pocketlytics
    proxy_cache_path /var/cache/nginx/pocketlytics
        levels=1:2
        keys_zone=pocketlytics_cache:10m
        max_size=100m
        inactive=60m
        use_temp_path=off;
...
server {
    ...
    location = /analytics/script.js {
        ...
        # Use the cache zone
        proxy_cache pocketlytics_cache;
        proxy_cache_valid 200 1h;
...
```bash
sudo mkdir -p /var/cache/nginx/pocketlytics
sudo chown www-data:www-data /var/cache/nginx/pocketlytics
```
...
Replace `YOUR_SITE_ID` with your actual site ID from the Pocketlytics dashboard.
...
4. **Open your website** in a browser with Developer Tools and verify:
   ...
   - Data appears in your Pocketlytics dashboard after 1-2 minutes

</Steps>

## How It Works

Nginx's `proxy_pass` directive transparently forwards requests to Pocketlytics's servers:

1. Browser requests `https://yourdomain.com/analytics/script.js`
2. Nginx matches the location block and forwards to `https://app.pocketlytics.local/api/script.js`
...
The Pocketlytics script auto-detects it's being served from your domain and sends all tracking data to your domain's `/analytics/*` endpoints.
...
location ~ ^/analytics/(script|replay|metrics)\.js$ {
    proxy_pass https://app.pocketlytics.local/api/$1.js;
    # ... proxy headers
    proxy_cache_valid 200 1h;
...
location ~ ^/analytics/(track|identify)$ {
    proxy_pass https://app.pocketlytics.local/api/$1;
    # ... proxy headers
...
Keep connections to Pocketlytics backend alive for better performance:

```nginx
upstream pocketlytics_backend {
    server app.pocketlytics.local:443;
    keepalive 32;
}

location /analytics/ {
    proxy_pass https://pocketlytics_backend/api/;
...
```nginx
location ~ ^/analytics/session-replay/record/ {
    proxy_pass https://app.pocketlytics.local/api/session-replay/record/;
    # ... proxy headers
...
**Solution**:
1. Check Pocketlytics backend is accessible:
   ```bash
   curl -I https://app.pocketlytics.local/api/script.js
   ```
...
### Incorrect geolocation in Pocketlytics
...
**Problem**: Logs show SSL verification errors when connecting to Pocketlytics.

**Solution**:
Add `proxy_ssl_server_name on` to enable SNI:
```nginx
location /analytics/ {
    proxy_pass https://app.pocketlytics.local/api/;
...
server {
    ...
    # Proxy all requests to Pocketlytics
    location / {
        proxy_pass https://app.pocketlytics.local/api/;
        proxy_set_header Host app.pocketlytics.local;
        proxy_set_header X-Real-IP $remote_addr;
...
### Multiple Pocketlytics Instances

If you need to proxy multiple Pocketlytics instances:

```nginx
location /analytics-prod/ {
    proxy_pass https://prod.pocketlytics.local/api/;
    # ... proxy config
}

location /analytics-staging/ {
    proxy_pass https://staging.pocketlytics.local/api/;
    # ... proxy config
}
```
...
- [Nginx Custom Configuration Guide](/docs/self-hosting-guides/custom-nginx) - Self-hosting Pocketlytics with Nginx
