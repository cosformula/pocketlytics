...
description: Common issues and solutions for Pocketlytics proxy configurations
---

import { Callout } from 'fumadocs-ui/components/callout';
import { Steps } from 'fumadocs-ui/components/steps';

This guide covers common issues you might encounter when setting up a Pocketlytics proxy and how to resolve them.
...
```
Your domain          →  Pocketlytics backend
-----------------        ---------------
...
Verify Pocketlytics backend is accessible:
...
- No tracking data appears in Pocketlytics dashboard
...
Tracking endpoints need proper CORS headers. Pocketlytics sets these automatically, but ensure your proxy preserves them:
...
// ✅ Correct - allow all origins (or use Pocketlytics's headers)
...
proxy_pass https://app.pocketlytics.local;
```
...
**Caddy:**
```txt
reverse_proxy https://app.pocketlytics.local {
...
// Next.js
{
  ...
  destination: `${process.env.NEXT_PUBLIC_POCKETLYTICS_HOST}/api/replay.js`,
}
```
...
```javascript
{
  ...
  destination: `${process.env.NEXT_PUBLIC_POCKETLYTICS_HOST}/api/session-replay/record/:siteId`,
}
```
...
**Symptoms:**
- Changed settings in Pocketlytics dashboard
...
sudo rm -rf /var/cache/nginx/pocketlytics/*
```

```javascript
// Node.js/Express with node-cache
cache.del(`pocketlytics_config_${siteId}`);
...
**Symptoms:**
- Pocketlytics released an update
...
```nginx
# Nginx with caching
proxy_cache_path /var/cache/nginx/pocketlytics levels=1:2 keys_zone=pocketlytics:10m;

location = /analytics/script.js {
    proxy_cache pocketlytics;
...
```bash
# Test latency to Pocketlytics
curl -w "@curl-format.txt" -o /dev/null -s https://app.pocketlytics.local/api/script.js
...
- Self-hosting Pocketlytics closer to your users
- Using a CDN proxy
...
```javascript
async rewrites() {
    console.log('POCKETLYTICS_HOST:', process.env.NEXT_PUBLIC_POCKETLYTICS_HOST);
    // Should log the URL
    return [...];
}
```
...
```bash
nslookup app.pocketlytics.local
```
...
```bash
curl -v https://app.pocketlytics.local/api/script.js
```
...
```bash
git add vercel.json
git commit -m "Add Pocketlytics proxy config"
git push
```
...
5. **Verify directly** against `https://app.pocketlytics.local/api/script.js`
