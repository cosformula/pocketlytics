...
description: Configure Laravel routes and middleware to proxy Pocketlytics tracking
---
...
Laravel makes it straightforward to proxy Pocketlytics tracking using routes and the HTTP client. This guide shows how to set up proxy endpoints in your Laravel application.

## Overview

Laravel's HTTP client (built on Guzzle) provides an elegant way to proxy requests to Pocketlytics servers while maintaining full control over headers, caching, and error handling.

**What you'll achieve:**
- Proxy all Pocketlytics endpoints through your Laravel app
- Forward necessary headers for accurate tracking
- Optional caching with Laravel Cache
- Support all Pocketlytics features
...
Add your Pocketlytics host to `.env`:

```bash
# .env
POCKETLYTICS_HOST=https://app.pocketlytics.local
# For self-hosted: POCKETLYTICS_HOST=https://analytics.yourcompany.com
```
...
{
    private string $pocketlyticsHost;

    public function __construct()
    {
        $this->pocketlyticsHost = config('services.pocketlytics.host', 'https://app.pocketlytics.local');
    }
    ...
    public function proxyScript(Request $request, string $script)
    {
        ...
        $cacheKey = "pocketlytics_script_{$script}";
    ...
    public function proxySiteConfig(Request $request, string $siteId)
    {
        ...
        $cacheKey = "pocketlytics_config_{$siteId}";
    ...
    private function forwardRequest(string $path, string $method, Request $request)
    {
        ...
        try {
            ...
            \Log::error('Pocketlytics proxy error', [
                'url' => $url,
                'error' => $e->getMessage(),
            ]);
...
Add Pocketlytics configuration to `config/services.php`:
...
return [
    ...
    'pocketlytics' => [
        'host' => env('POCKETLYTICS_HOST', 'https://app.pocketlytics.local'),
    ],
];
```
...
4. **Verify in Pocketlytics dashboard**: Data should appear

</Steps>

## How It Works

Laravel routes intercept requests to `/analytics/*` and forward them to Pocketlytics:

1. Request to `/analytics/script.js` hits Laravel route
2. Controller forwards to `https://app.pocketlytics.local/api/script.js`
...
public function proxySessionReplay(Request $request, string $siteId)
{
    ...
    ForwardAnalyticsData::dispatch(
        "{$this->pocketlyticsHost}/api/session-replay/record/{$siteId}",
        $request->all(),
        [
...
**Problem**: Old script cached after Pocketlytics update.

**Solution**:
Clear specific cache key:
```bash
php artisan cache:forget pocketlytics_script_script.js
```
...
Cache::tags(['pocketlytics', 'scripts'])->remember($cacheKey, 3600, function () {
    // ... fetch script
});

// Clear all Pocketlytics cache
Cache::tags(['pocketlytics'])->flush();
```
...
Http::pool(fn (Pool $pool) => [
    $pool->get("{$this->pocketlyticsHost}/api/script.js"),
    $pool->get("{$this->pocketlyticsHost}/api/replay.js"),
]);
```
