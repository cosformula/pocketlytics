...
description: Configure Next.js rewrites to proxy Pocketlytics tracking and bypass ad blockers
---
...
Next.js provides a powerful [rewrites](https://nextjs.org/docs/app/api-reference/next-config-js/rewrites) feature that makes it easy to proxy Pocketlytics tracking through your own domain. This guide works with both the App Router (Next.js 13+) and Pages Router.

## Overview

Next.js rewrites allow you to map incoming request paths to different destination URLs transparently. This is perfect for proxying analytics since the browser sees requests to your domain while Next.js forwards them to Pocketlytics's servers behind the scenes.

**What you'll achieve:**
- Serve Pocketlytics scripts from your domain (e.g., `/analytics/script.js`)
...
Create or update your `.env` or `.env.local` file with your Pocketlytics instance URL:
...
```bash
# .env.local
NEXT_PUBLIC_POCKETLYTICS_HOST=https://app.pocketlytics.local
```
...
```bash
# .env.local
NEXT_PUBLIC_POCKETLYTICS_HOST=https://analytics.yourcompany.com
```

Replace `analytics.yourcompany.com` with your actual Pocketlytics instance domain.
...
This configuration proxies all Pocketlytics endpoints including session replay and Web Vitals:
...
const nextConfig = {
  async rewrites() {
    const pocketlyticsHost = process.env.NEXT_PUBLIC_POCKETLYTICS_HOST;

    if (!pocketlyticsHost) {
      console.warn('NEXT_PUBLIC_POCKETLYTICS_HOST is not set. Pocketlytics proxy will not work.');
      return [];
    }
    ...
    return [
      // Tracking scripts
      {
        ...
        destination: `${pocketlyticsHost}/api/script.js`,
      },
      {
        source: '/analytics/replay.js',
        destination: `${pocketlyticsHost}/api/replay.js`,
      },
      {
        source: '/analytics/metrics.js',
        destination: `${pocketlyticsHost}/api/metrics.js`,
      },
      // Tracking endpoints
      {
        source: '/analytics/track',
        destination: `${pocketlyticsHost}/api/track`,
      },
      {
        source: '/analytics/identify',
        destination: `${pocketlyticsHost}/api/identify`,
      },
      {
        source: '/analytics/session-replay/record/:siteId',
        destination: `${pocketlyticsHost}/api/session-replay/record/:siteId`,
      },
      {
        source: '/analytics/site/tracking-config/:siteId',
        destination: `${pocketlyticsHost}/api/site/tracking-config/:siteId`,
      },
    ];
  },
};
...
<Callout type="info">
**Custom Path**: You can change `/analytics` to any path you prefer (e.g., `/tracking`, `/stats`, `/pocketlytics`). Just ensure you use the same path consistently in both your rewrites and script tag.
</Callout>

### Update Your Tracking Script

Update your script tag to load from the proxied path instead of directly from Pocketlytics:
...
2. **Verify the script loads**:
   - You should see a request to `/analytics/script.js` (not `app.pocketlytics.local`)
...
3. **Verify tracking works**:
   ...
   - All requests should show your domain, not `app.pocketlytics.local`
4. **Check the Pocketlytics dashboard**:
...
The Pocketlytics tracking script has a built-in auto-discovery mechanism. When it loads from:
...
This means all tracking requests automatically go to `/analytics/track`, `/analytics/identify`, etc., which Next.js then rewrites to forward to Pocketlytics's servers.
...
The Pocketlytics server sets appropriate cache headers, which Next.js respects:
...
4. Ensure your Pocketlytics instance URL is correct in `.env.local`
...
**Problem**: Rewrites don't work, console shows warning about missing `NEXT_PUBLIC_POCKETLYTICS_HOST`.
...
1. Add `NEXT_PUBLIC_POCKETLYTICS_HOST` environment variable in your Vercel project settings
