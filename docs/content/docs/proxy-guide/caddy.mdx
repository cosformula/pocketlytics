...
description: Configure Caddy reverse proxy to serve Pocketlytics tracking with automatic HTTPS
---
...
Caddy is a powerful web server with automatic HTTPS that makes reverse proxying simple and secure. This guide shows how to configure Caddy to proxy Pocketlytics tracking through your own domain.
...
yourdomain.com {
    ...
    handle /analytics/script.js {
        reverse_proxy https://app.pocketlytics.local {
            header_up Host app.pocketlytics.local
...
This proxies all `/analytics/*` requests to `https://app.pocketlytics.local/api/*`.
...
3. **Open your website** in a browser with Developer Tools and verify:
   ...
   - Data appears in Pocketlytics dashboard

</Steps>

## How It Works

Caddy's `reverse_proxy` directive forwards requests to Pocketlytics's servers:
...
4. Forwards to `https://app.pocketlytics.local/api/script.js` with proper headers
5. Returns response to browser

The Pocketlytics script auto-detects it's being served from your domain and sends all tracking to your domain's endpoints.
...
{$DOMAIN:yourdomain.com} {
    handle_path /analytics/* {
        reverse_proxy {$POCKETLYTICS_HOST:https://app.pocketlytics.local} {
...
export POCKETLYTICS_HOST=https://app.pocketlytics.local
caddy run
```
...
**Solution**:
1. Verify Pocketlytics backend is accessible:
   ```bash
...
**Problem**: Requests go to wrong Pocketlytics endpoints.
