...
```bash filename="npm"
npm install @pocketlytics/node
```

```bash filename="yarn"
yarn add @pocketlytics/node
```
...
```typescript
import { Pocketlytics } from "@pocketlytics/node";

const pocketlytics = new Pocketlytics(config: PocketlyticsConfig);
```
...
const pocketlytics = new Pocketlytics({
...
The `Pocketlytics` constructor will throw an error if required configuration options (`analyticsHost`, `siteId`, `apiKey`) are missing or invalid.
</Callout>
...
| :-------------- | :----------------- | :------ | :----------------------------------------------------------------------------------------------- |
| `analyticsHost` | `string`           | -       | **Required.** URL of your Pocketlytics analytics instance (e.g., `https://pocketlytics.yourdomain.com/api`). |
| `siteId`        | `string \| number` | -       | **Required.** The Site ID for your website obtained from your Pocketlytics instance.                   |
...
```typescript
await pocketlytics.pageview(payload?: Payload);
```
...
```typescript
await pocketlytics.event(eventName: string, payload?: Payload, properties?: TrackProperties);
```
...
async function registerUser(userData, req) {
  ...
  await pocketlytics.event("user_registered",
    {
...
async function processDailyReport(reportId) {
  ...
  await pocketlytics.event("report_generated",
    {},
    {
      reportId: reportId,
      status: "success",
