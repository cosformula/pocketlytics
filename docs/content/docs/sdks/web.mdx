...
description: Official JavaScript SDK for Pocketlytics Analytics
---
...
```bash
npm install @pocketlytics/js
```

```bash
yarn add @pocketlytics/js
```
...
<Callout type="info">
The `rrweb` package is only required if you enable session replay in your Pocketlytics dashboard. The SDK will work without it for all other tracking features.
</Callout>

## Initialization

<Callout type="warning">
`pocketlytics.init()` **must** be called once before any other tracking methods (`pageview`, `event`, etc.) can be used. Attempting to call other methods before `init` will result in errors or no tracking.
</Callout>

### Syntax

```typescript
async pocketlytics.init(config: PocketlyticsConfig): Promise<void>;
```

### Example

```javascript
import pocketlytics from "@pocketlytics/js";

await pocketlytics.init({
  analyticsHost: "https://app.pocketlytics.local/api",
  siteId: "1",
});
```
...
| :-------------- | :------- | :----------------------------------------------------------------------------------- |
| `analyticsHost` | `string` | URL of your Pocketlytics analytics instance (e.g., `https://pocketlytics.yourdomain.com/api`). |
| `siteId`        | `string` | The Site ID for your website obtained from your Pocketlytics instance.       |
...
These settings are controlled through your Pocketlytics dashboard and cannot be set in the init config:
...
```typescript
pocketlytics.pageview(path?: string);
```
...
function openUserDetailsModal(userId, step) {
  ...
  pocketlytics.pageview(`/users/${userId}/modal/${step}`);
}
...
function showFilteredProductList(category, sortBy) {
   ...
   pocketlytics.pageview(virtualPathWithQuery);
}
...
```typescript
pocketlytics.event(name: string, properties?: TrackProperties);
```
...
The SDK automatically listens for clicks on elements with the `data-pocketlytics-event` attribute.

- `data-pocketlytics-event="<event-name>"`: This attribute makes the element trackable. The value is used as the event name.
- `data-pocketlytics-prop-<property-name>="<value>"`: You can add multiple properties to the event by using this attribute format. The SDK collects all attributes starting with `data-pocketlytics-prop-` and includes them as key-value pairs in the event data.

#### Example

```html
<button data-pocketlytics-event="cta_click">Click Me</button>

<button
  data-pocketlytics-event="add_to_cart"
  data-pocketlytics-prop-item_id="product-123"
  data-pocketlytics-prop-item_name="Classic T-Shirt"
  data-pocketlytics-prop-price="29.99"
>
  Add to Cart
</button>
```
...
```typescript
pocketlytics.trackOutbound(url: string, text?: string, target?: string);
```
...
document.getElementById("external-service-button")?.addEventListener("click", () => {
  ...
  pocketlytics.trackOutbound(url, "Visit External Service", "_blank");
...
The SDK can automatically capture JavaScript errors and unhandled promise rejections. This feature must be enabled in your Pocketlytics dashboard.
...
```typescript
pocketlytics.error(error: Error | ErrorEvent, context?: TrackProperties);
```
...
Session replay must be enabled in your Pocketlytics dashboard under. Once enabled, recording starts automatically when the SDK initializes.
...
pocketlytics.startSessionReplay();

// Stop recording
pocketlytics.stopSessionReplay();

// Check if currently recording
const isRecording = pocketlytics.isSessionReplayActive();
if (isRecording) {
...
```typescript
pocketlytics.identify(userId: string, traits?: Record<string, unknown>);
```
...
pocketlytics.identify("user-xzy-123");
...
```typescript
pocketlytics.setTraits(traits: Record<string, unknown>);
```
...
```typescript
pocketlytics.getUserId(): string | null;
```

**Example:**

```javascript
let currentUserId = pocketlytics.getUserId();
console.log("Current Pocketlytics User ID:", currentUserId);
```
...
```typescript
pocketlytics.onPageChange(callback: (newPath: string, previousPath: string) => void): () => void;
```
...
```javascript
// Register a callback
const unsubscribe = pocketlytics.onPageChange((newPath, previousPath) => {
...
pocketlytics.onPageChange((newPath) => {
  // Update page title
  document.title = `${newPath} - My Site`;
});

pocketlytics.onPageChange((newPath, previousPath) => {
...
localStorage.setItem("disable-pocketlytics", "true");

// Opt out via window flag (current session only)
window.__POCKETLYTICS_OPTOUT__ = true;
```

To re-enable tracking, remove the localStorage item and reload the page:

```javascript
localStorage.removeItem("disable-pocketlytics");
```
