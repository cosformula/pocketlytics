  ...
  try {
    ...
    if (!email) {
      if (isGetRequest) {
        return reply.status(400).type("text/html").send(`
          <!DOCTYPE html>
          <html><head><title>Unsubscribe - Pocketlytics</title></head>
    ...
    if (isGetRequest) {
      return reply.status(200).type("text/html").send(`
        <!DOCTYPE html>
        <html><head><title>Unsubscribed - Pocketlytics</title></head>
        <body style="font-family: sans-serif; max-width: 600px; margin: 50px auto; text-align: center;">
          <h1>Unsubscribed</h1>
          <p>You have been successfully unsubscribed from Pocketlytics marketing emails.</p>
