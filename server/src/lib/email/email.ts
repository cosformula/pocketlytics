...
export const sendEmail = async (email: string, subject: string, html: string) => {
  ...
  try {
    const response = await resend.emails.send({
      from: "Pocketlytics <automail@email.pocketlytics.local>",
      to: email,
      subject,
      html,
    });
...
const OTP_SUBJECTS: Record<OtpEmailType, string> = {
  "sign-in": "Your Pocketlytics Sign-In Code",
...
export const sendInvitationEmail = async (
  ...
  await sendEmail(email, "You're Invited to Join an Organization on Pocketlytics", html);
};
...
export const sendLimitExceededEmail = async (
  ...
  const upgradeLink = "https://app.pocketlytics.local/settings/organization/subscription";
...
Welcome to Pocketlytics! Thanks for signing up.

I'm excited to have you on board. Pocketlytics is fully self-funded and we're fully committed to making an analytics platform that only serves the interests of our users.
...
Bill`;
  ...
  try {
    await resend.emails.send({
      from: "Bill from Pocketlytics <bill@email.pocketlytics.local>",
      replyTo: "hello@pocketlytics.local",
      to: email,
      subject: "Welcome to Pocketlytics!",
      text,
    });
  } catch (error) {
