...
import { PocketlyticsEvent } from "./pocketlytics.js";
import { z } from "zod";
...
export class SimpleAnalyticsImportMapper {
  ...
  static transform(events: SimpleAnalyticsEvent[], site: number, importId: string): PocketlyticsEvent[] {
    return events.reduce<PocketlyticsEvent[]>((acc, event) => {
