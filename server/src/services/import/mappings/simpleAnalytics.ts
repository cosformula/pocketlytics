...
const logger = createServiceLogger("import:simple-analytics-mapper");
...
export const simpleAnalyticsHeaders = [
  "added_iso",
  "country_code",
  "datapoint",
  "document_referrer",
...
export class SimpleAnalyticsImportMapper {
  ...
  static transform(events: SimpleAnalyticsEvent[], site: string, importId: string): RybbitEvent[] {
    return events.reduce<RybbitEvent[]>((acc, event) => {
      ...
      if (!parsed.success) {
        logger.error({ site, importId, error: parsed.error.flatten() }, "Invalid Simple Analytics event");
        return acc;
      }
