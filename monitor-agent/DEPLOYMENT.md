# Pocketlytics Monitor Agent Deployment Guide

This guide covers deploying the Pocketlytics Monitor Agent to VPS instances in different regions.
...
- Main Pocketlytics server IP address for whitelisting
...
sudo mkdir -p /opt/pocketlytics-monitor-agent
cd /opt/pocketlytics-monitor-agent
...
wget https://raw.githubusercontent.com/cosformula/pocketlytics/main/monitor-agent/docker-compose.yml
wget https://raw.githubusercontent.com/cosformula/pocketlytics/main/monitor-agent/docker-compose.prod.yml
wget https://raw.githubusercontent.com/cosformula/pocketlytics/main/monitor-agent/Caddyfile
...
sudo nano /opt/pocketlytics-monitor-agent/.env

# Restart the service
sudo systemctl restart pocketlytics-monitor-agent
```
...
Add the agent regions to your main Pocketlytics server database:
...
3. **Main Server**: Check region health in the Pocketlytics UI
...
# Systemd deployment
sudo journalctl -u pocketlytics-monitor-agent -f
sudo systemctl status pocketlytics-monitor-agent
```
...
cp /opt/pocketlytics-monitor-agent/.env /opt/pocketlytics-monitor-agent/.env.backup
```
